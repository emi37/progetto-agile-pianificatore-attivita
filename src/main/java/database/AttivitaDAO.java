...
public class AttivitaDAO {
    ...
    /**
     * Inserisce l'attività e restituisce l'ID generato dal db (per collegare le notifiche).
     */
    public int inserisciAttivitaRestituendoId(Attivita attivita, int idUtente, int idCategoria, int idPriorita) {
        ...
        try (Connection connessione = DatabaseManager.getConnection(); 
            ...
            ps.setNull(4, Types.TIMESTAMP); // È nuova, quindi nson completata
            ...
            if (righeSalvate > 0) {
                // prendiamo l'id generato dal database
        ...
        } catch (SQLException e) { //eccezione
            System.err.println("Impossibile generare l'id durante l'inserimento dell'attività!");
            e.printStackTrace();
            return -1;
        }
    }
